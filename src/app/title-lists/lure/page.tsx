    ...
    const fetchTrophies = async () => {
      try {
        const response = await axiosInstance.get<TrophySearch>(
          `/trophies/search/COLOR/${lureType}`
        );
        ...
        <TableContainer>
          <Table variant="unstyled">
            ...
            <Tbody>
              {!isLoading &&
                trophies &&
                trophies.results.map((trophy, index) => (
                  <Tr key={index}>
                    <TitleListTd>
                      <Flex
                        direction="column"
                        alignItems="center"
                        gap="1"
                        onClick={() => onClickLure(trophy.lureId)}
                      >
                        {trophy.lureImagePath ? (
                          <Avatar
                            src={trophy.lureImagePath}
                            size="md"
                            name={trophy.lureType}
                            rounded="md"
                          />
                        ) : (
                          <Avatar src={defaultImage.src} size="md" name={''} rounded="md" />
                        )}
                        <Text maxW="100px" fontSize="2xs" py="1" lineHeight="1.5" isTruncated>
                          {lureType} <br />#{trophy.color}
                        </Text>
                      </Flex>
                    </TitleListTd>
                    <TitleListTd pt="1">
                      {trophy.userName ? (
                        <Flex
                          ...
                          alignItems="center"
                          gap="1"
                          onClick={() => onClickUser(trophy.userName)}
                        >
                          <Avatar src={trophy.profileIcon} size="md" name={trophy.userName} />
                          <Text maxW="100px" fontSize="xs" isTruncated>
                            @{trophy.userName}
                          </Text>
                        </Flex>
                      ) : (
            ...
            {!isLoading && trophies?.summary === 0 && (
              <TableCaption textAlign="left" pt="0">
                {t('カラータイトルがありません。')}
              </TableCaption>
            )}
            {isLoading && (
